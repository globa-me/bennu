...
export function useSiteLoader(loadDocument, setStatus) {
  const handleOpenFile = useCallback(async (event) => {
    ...
    try {
      if (isZipFile(file)) {
        ...
        setStatus("Loaded single HTML. Local assets will be saved to temporary session.");
      }
    } catch (error) {
  ...
  const handleOpenDirectory = useCallback(async (event) => {
