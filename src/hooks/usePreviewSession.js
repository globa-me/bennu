...
export function usePreviewSession({
  ...
  const normalizeElement = useCallback(
    (element) => {
      const session = siteSessionRef.current;
  ...
  const handleMessage = useCallback(
    (event) => {
      ...
      if (
        message.source !== "bennu-preview" ||
        message.token !== previewTokenRef.current
      )
        return;
      ...
      if (message.type === "select") {
        ...
        setStatus(
          message.element
            ? t("app.selected", { tag: message.element.tagName })
  ...
  const updateSelected = useCallback(
    (action, payload = {}) => {
      ...
      if (
        action === "set-attr" &&
        ["src", "href", "poster", "data", "xlink:href"].includes(
          payload.name,
        ) &&
        payload.value
      ) {
        nextPayload.value =
          siteSessionRef.current?.previewUrlFor(payload.value) || payload.value;
      }
      postToPreview({
        action,
        id: payload.id || selectedElement.id,
        ...nextPayload,
      });
    },
